...
const { commit } = Import({ channel });
```

如果你使用 TypeScript，可以指定一个泛型参数，表示从另一个运行环境中导出的 JS 对象的类型，以便编辑器提供完整的代码高亮提示。

```ts
const { commit } = Import<Window>({ channel });
```

- 导入的信道模块提供了 commit 方法，该方法接收一个记录器函数作为参数，用于记录和提交调用信息。
- 记录器函数接收一个可调用的代理作为输入参数，对该代理执行的操作将被发送至导出模块的上下文进行解释，也就是应用到模块上。
- 解释结果将作为 commit 方法所返回的 Promise 的兑现参数。如果解释过程中出现错误，则该错误将成为 Promise 被拒绝的原因。

如果这些内容暂时看起来有点复杂，可以先忽略，后续文档会提供具体示例。
...
对象导出到了广播信道，也相互从广播信道导入了信道模块，与对方窗口导出的 `window` 对象对应。

接下来，我将以此为例，直接在你的浏览器中演示信道模块为这两个窗口提供的跨 JS 运行环境的模块调用能力，请暂时不要关闭新打开的窗口。

你可以编辑接下来的文档提供的输入框内的代码，并点击运行按钮进行测试。示例中的 `channel`、`commit`、`set`
...
```js
await commit(otherWindow => otherWindow.innerWidth);
```
